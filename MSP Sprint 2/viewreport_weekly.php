<?php
	...
	define('TITLE', 'Produce Weekly Sales Report');
	//header
	require('header.php');
	require('dbcon.php');

	//body
	echo "<h1>Produce a Weekly Sales Report</h1>";
	...
	if (isset($_POST['submitted'])) {
		...
		while ($rows = mysqli_fetch_array($myData)) {
			...
			$query2 = "SELECT * FROM salesrecord WHERE (date BETWEEN '".$startDate."' AND '".$endDate."') AND status=1";
		...
		if (count($arr) > 0) {
			...
			echo "<p>No items were sold in this week.</p>";
		}
		...
		//title
		echo "<h4>Please select a start date for the sales report</h4>";
		echo "<p>An end date will be selected automatically.</p>";
		
		//display the form
		echo "<form action='viewreport_weekly.php' method='post'>";
