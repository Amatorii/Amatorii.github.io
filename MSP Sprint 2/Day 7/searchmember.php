<?php
	define('TITLE', 'Search Member');
	//header
	require('header.php');
	require('dbcon.php');

	//body
	echo "<h1>Search for Member</h1>";
	...
	if (isset($_POST['submitted'])) {
		...
		if (!empty($memberId)) {
			if (!is_numeric($memberId)) {
				...
				echo "<p><a href='searchmember.php'>Try Again</a></p>";
			}
			else {
				$validId = true;
			}
		...
		if ($validId) {
			...
			if ($myData) {
				...
				if ($row > 0) {
					...
					echo "</tr>";
					echo "</table>";
					echo "<p><a href='searchmember.php'>Search for another member</a></p>";
				}
				else {
		...
		//display the form
		echo "<form action='searchmember.php' method='post'>";
